...
Seven factors:
    ...
    value_alignment  match to value priors
...
Theory base: value-directed remembering (Castel), levels of processing
(Craik & Lockhart), emotional consolidation (McGaugh), rational analysis
of memory (Anderson & Schooler 1991), and RL / expected-utility — value =
marginal contribution to future task success.

Weights `w_i` are LEARNED from downstream task return by a gradient-free
optimiser (`learn_weights`), since the encode→forget→retrieve→answer
pipeline is non-differentiable.
"""
...
def default_memory_value(override: dict | None = None) -> "MemoryValue":
    ...
    return MemoryValue(weights={**DEFAULT_WEIGHTS, **(override or {})})
...
def memory_factors(row: dict[str, Any]) -> dict[str, float]:
    """
    Extract the 7-factor dict from a memory row.
...
def value_forget_score(
    ...
    *,
    ...
    High memory value V resists forgetting. Recency + usage are kept as
    the universal time/access dynamics.
    """
    if now is None:
        now = datetime.now()
