...
Many agents keep their memory in Markdown, not canonical JSON. This module
reads such a dump into ``MemoryRecord`` objects so the downstream value
...
def load_markdown(path: str | Path, *, split: str = "auto") -> list[MemoryRecord]:
