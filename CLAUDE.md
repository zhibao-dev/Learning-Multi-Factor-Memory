...
- **Branch defines the memory model (paper separation).** On the **`multi-factor-eval`** branch (paper2), one shared `MemoryValue` (`borge/memory/value.py`, weights from `default_memory_value`, config key `borge.memory.value.weights`) drives all three memory ops: encoding depth (`value_encoding_depth`), forget score (`value_forget_score`), retrieval rank (`w_v·V + query terms`). The seven factors are computed+persisted at encode (consolidation Step 3) and read at forget/retrieve; `task_utility` is LLM-gated (0 by default). On **`main`** and **`self-FEP`** (paper1) the same code runs the product-of-resistances forget formula + the 5-term recall + ΔF→importance loop described in the last bullet. **Do not cross-merge `borge/memory/` between these branches** — the divergence is intentional.
