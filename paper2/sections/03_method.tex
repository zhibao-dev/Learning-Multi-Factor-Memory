...
factor vector $\fvec(m)\in[0,1]^7$ and scored by a linear value
\begin{equation}
...
(\cref{tab:factors}). Linearity is a deliberate choice: it keeps the
policy low-dimensional, lets us read each factor's learned contribution
directly from $w_i$, and makes the value cheap to evaluate at every
consolidation step. More generally the value is any scoring function
$V(m)=g_\theta(\fvec(m))$; \cref{eq:value} is its interpretable
linear instance, and a small neural $g_\theta$ is the natural nonlinear
extension once interpretability can be traded for capacity
