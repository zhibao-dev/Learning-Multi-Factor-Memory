...
(Self-relevance is the best \emph{standalone} factor in \cref{tab:blind}
yet reliability carries the larger learned weight: the weights measure
each factor's marginal contribution \emph{within} the learned mixture, not
its solo ranking power.)
...
only $55\%$ of splits, i.e.\ a statistical tie. On these four live factors
the nonlinear capacity buys nothing: the factors combine near-additively,
so the interpretable linear model is not a compromise but the right choice.
