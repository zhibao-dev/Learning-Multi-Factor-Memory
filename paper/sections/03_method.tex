...
\paragraph{Scope: a variationally-inspired reliability link.}
...
update rule \cref{eq:pi} is the simplest closed-form link that:
(i) maps observed prediction-error variance $\Var[\PE]$ into $[0,1]$;
(ii) returns $1$ when $\Var[\PE] \to 0$ (perfectly reliable self prior)
and $\to 0$ as $\Var[\PE] \to \infty$ (uninformative self prior); and
(iii) has a single calibration hyperparameter $\gamma$ that we interpret
as the agent's \emph{prior precision on $\Var[\PE]$ under a reliable
hypothesis}. Large $\gamma$ encodes the prior belief that a working
self model leaves only tiny PE residuals; small $\gamma$ admits more
noisy self-evidence before $\piself$ collapses. We expose the
hyperparameter in code under the explicit name
\texttt{prior\_precision} (with \texttt{gamma} as a legacy alias).
The form recovers (in the limit of large rolling window) a regularised
inverse-variance precision estimate, which is the same functional shape
the Gamma--Inverse-Gamma conjugate update gives under Gaussian
observation noise. We do \emph{not} claim \cref{eq:pi} is the exact
variational posterior of a fully specified generative model --- a
strict-derivation treatment is deferred to follow-up work and named
explicitly in \cref{sec:limitations}.
...
+ w_f\,\Delta F_i + w_s\,\mathrm{self}_i,
\end{equation}
where $\mathrm{self}_i$ is the encoding-specificity-faithful similarity
\[
\mathrm{self}_i = \tfrac{1}{2}\bigl(1 + \cos(\muself^{\text{enc},i}, \muself^{\text{now}})\bigr),
\]
with $\muself^{\text{enc},i}$ the snapshot of $\muself$ at the moment
memory $i$ was formed (persisted per row; see ``Implementation''
below). For legacy rows that lack the snapshot we fall back to
$\tfrac{1}{2}(1 + \cos(e_i, \muself^{\text{now}}))$. Default weights
$(w_m,w_r,w_q,w_f,w_s){=}(0.35,0.15,0.25,0.10,0.15)$. Each retrieved
memory's count $r_i$ is incremented, closing the retrieval
$\leftrightarrow$ forgetting loop (more recall makes a memory harder
to forget on the next sweep).
...
We additionally persist the live $\muself$ centroid as
\texttt{mu\_self\_at\_encoding} (JSON-encoded), enabling the
encoding-specificity term in \cref{eq:rank} (Tulving 1973).
...
all components ship in a single SQLite table extended with three columns
(\texttt{self\_relevance\_score}, JSON \texttt{embedding}, and JSON
\texttt{mu\_self\_at\_encoding}) plus the \texttt{SelfModel} class. Existing test coverage (30 cases) passes
unchanged; we add 7 cases that pin the new behaviour. The substrate
and reproducibility scripts are open-source
\cite{anon_substrate}.
