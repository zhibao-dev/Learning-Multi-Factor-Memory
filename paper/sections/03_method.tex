...
\paragraph{Scope: precision-inspired, not variational.}
Before describing the mechanism, we make the FEP scope precise.
We use the FEP as a \emph{design discipline}: the self is treated as a
generative model with a precision-weighted prior, and self-precision
gates downstream processing the way precision does throughout the FEP
literature. We do \emph{not} derive the precision update rule from a
variational free-energy objective; \cref{eq:pi} is engineered to have
the qualitative shape FEP precision should have (low PE variance
$\Rightarrow$ high precision) and is a first commitment, not a
theorem. A variational derivation under a specified likelihood-prior
pair is named explicitly as a follow-up step
(\cref{sec:limitations}, L2). Where the paper says ``FEP-inspired'' or
``FEP-grounded mechanism,'' it means: faithful to the precision-gating
intuition; not yet faithful to variational free-energy minimisation as
an objective.
